...
namespace ImGui
{
    ...
    IMGUI_API void          SetColumnWidth(int column_index, float imageWidth);                      // set column width (in pixels). pass -1 to use current column
...
struct ImGuiTextFilter
{
    ...
    IMGUI_API bool      Draw(const char* label = "Filter (inc,-exc)", float imageWidth = 0.0f);  // Helper calling InputText+Build
...
struct ImFontAtlas
{
    ...
    IMGUI_API int               AddCustomRectRegular(int imageWidth, int height);
    IMGUI_API int               AddCustomRectFontGlyph(ImFont* font, ImWchar id, int imageWidth, int height, float advance_x, const ImVec2& offset = ImVec2(0, 0));
