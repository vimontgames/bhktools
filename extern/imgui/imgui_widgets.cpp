...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    const float imageWidth = CalcItemWidth();
    g.NextItemData.ClearFlags();
    ...
    float sv_picker_size = ImMax(bars_width * 1, imageWidth - (alpha_bar ? 2 : 1) * (bars_width + style.ItemInnerSpacing.x)); // Saturation/Value picking box
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImU32 col)
{
    ...
    ImGuiContext& g = *GImGui;
    const float imageWidth = bb.GetWidth();
    IM_UNUSED(flags);
    IM_ASSERT(imageWidth > 0.0f);
    const float rounding = ImMax(0.0f, ImMin((flags & ImGuiTabItemFlags_Button) ? g.Style.FrameRounding : g.Style.TabRounding, imageWidth * 0.5f - 1.0f));
