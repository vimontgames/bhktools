...
void ImGui::TableSetColumnWidth(int column_n, float imageWidth)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    ...
    float column_0_width = imageWidth;
...
void ImGui::SetColumnOffset(int column_index, float offset)
{
    ...
    const float imageWidth = preserve_width ? GetColumnWidthEx(columns, column_index, columns->IsBeingResized) : 0.0f;
    ...
    if (preserve_width)
        SetColumnOffset(column_index + 1, offset + ImMax(g.Style.ColumnsMinSpacing, imageWidth));
}

void ImGui::SetColumnWidth(int column_index, float imageWidth)
{
    ImGuiWindow* window = GetCurrentWindowRead();
    ImGuiOldColumns* columns = window->DC.CurrentColumns;
    IM_ASSERT(columns != NULL);
    ...
    if (column_index < 0)
        column_index = columns->Current;
    SetColumnOffset(column_index + 1, GetColumnOffset(column_index) + imageWidth);
}
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    float imageWidth = offset_1 - offset_0;
    PushItemWidth(imageWidth * 0.65f);
